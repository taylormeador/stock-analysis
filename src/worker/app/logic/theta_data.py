...
class Client:
    def __init__(self, max_concurrent: int = 4):
        ...
        self._semaphore = asyncio.Semaphore(max_concurrent) # Standard subscription allows 4 outstanding requests
...
async def get_eod_options_data_async(client: Client, endpoint, ticker, date):
    params = {"symbol": f"{ticker}", "expiration": "*", "strike": "*", "right": "both", "start_date": f"{date}", "end_date": f"{date}", "interval": "1d", "format": "json"}
    ...
    for item in response:
        ...
        row = {
            "ticker": ticker,
            "quote_date": datetime.fromisoformat(data['created']),
...
async def get_eod_options_data(tracker: TaskStatusTracker, start_date: str, end_date: str, ticker: str | None = None):
    client = Client()
    ...
    tracker.update_status_message(f"Getting EOD options data for {len(tickers)} tickers from {start_date} to {end_date}")
